                        ...
                        <div class="card-header" style="border-bottom: 0">
                            ...
                            <div class="float-right" style="margin-right: 10px">
                                <a href="{{ route('admin.product-categories.index') }}">
                                    <button class="btn btn-info"><i class="fa fa-plus-circle"></i> Danh mục</button>
                                </a>
                                    ...
                                    @if(count($products)>0)
                                        @foreach($products as $product)
                                            <tr class="remove-{{ $product->id }}">
                                                <td class="center">
                                                    <div class="custom-control custom-checkbox">
                                                        <input class="custom-control-input ace mycheckbox"
                                                               ...
                                                               value="{{ $product->id }}" name="check[]">
                                                        <label for="checkall-{{ $product->id }}"
                                                ...
                                                <td>{{ $product->title }}</td>
                                                {{--                                                <td>{!! \App\Models\Config::showCategoryProduct($product->id) !!}</td>--}}
                                                <td>{!! \App\Models\Config::getPriceProduct($product->regular_price,$product->sale_price) !!}</td>
                                                <td>
                                                    Hãng: {!! \App\Models\Config::ProductBrands($product->brands_id) !!}
                                                    <br>SL tổng: {{$product->quantity }},
                                                    <br>Xem: {{$product->view}},
                                                    <br>Mua: {{$product->number_sell }}

                                                </td>
                                                <td>
                                                    <div data-table="products" data-id="{{ $product->id }}"
                                                         data-col="is_hot"
                                                         class="Switch Round  @if($product->is_hot == 1) On @else Off @endif"
                                                ...
                                                <td>
                                                    <div data-table="products" data-id="{{ $product->id }}"
                                                         data-col="is_active"
                                                         class="Switch Round  @if($product->is_active == 1) On @else Off @endif"
                                                         style="vertical-align:top;margin-left:10px;">
                                                        <div class="Toggle"></div>
                                                    </div>
                                                </td>
                                                <td>{{ date_format($product->created_at,'d/m/Y H:i:s') }}
                                                    <br>Người tạo
                                                    : {!! \App\Models\Config::showAuthor($product->id_user) !!}
                                                    <br> Danh mục
                                                ...
                                                <td>
                                                    ...
                                                    <a href="#" name="{{ $product->title }}"
                                                       link="{{ route('admin.products.destroy',$product->id) }}"
                                                       ...
                                                       data-target="#deleteModal" style="margin-bottom: 3px"> <span
                                                            class="btn btn-danger btn-sm"><i title="Delete"
                                                                                             class="ace-icon fa fa-trash-o bigger-130"></i></span></a><br><br>

                                                    <a href="{{url('/'.$product->slug)}}.html"
                                                       class="btn btn-sm btn-warning"><i class="">xem nhanh</i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
                                            <td colspan="12" class="text-center">Dữ liệu đang cập nhật</td>
                                        </tr>
                                    @endif
                                    </tbody>
                                </table>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $(".deleteClick").click(function () {
                ...
                $("#deleteMes").html("Bạn muốn xoá sản phẩm <b>" + name + "</b> ?");
            });
