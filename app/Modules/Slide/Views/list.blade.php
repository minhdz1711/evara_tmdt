                                ...
                                <table id="example1" class="table table-bordered table-striped dataTable">
                                    ...
                                    @if(count($slides)>0)
                                        @foreach($slides as $slide)
                                            <tr class="remove-{{ $slide->id }}">
                                                <td class="center">
                                                    <div class="custom-control custom-checkbox">
                                                        <input class="custom-control-input ace mycheckbox"
                                                               type="checkbox"
                                                               id="checkall-{{ $slide->id }}"
                                                               value="{{ $slide->id }}" name="check[]">
                                                        <label for="checkall-{{ $slide->id }}"
                                                               class="custom-control-label"></label>
                                                    </div>
                                                </td>
                                                <td style="width: 120px">
                                                    <div class="box-image-table">
                                                        <img src="{!! ($slide->images!="")?url($slide->images):URL::asset('images/no_picture.gif') !!}"
                                                             alt="{{ $slide->title }}"
                                                             class="img-table">
                                                    </div>
                                                </td>
                                                ...
                                                <td>
                                                    <div data-table="slides" data-id="{{ $slide->id }}"
                                                         data-col="is_active"
                                                         class="Switch Round  @if($slide->is_active == 1) On @else Off @endif"
                                                         style="vertical-align:top;margin-left:10px;">
                                                        <div class="Toggle"></div>
                                                    </div>
                                                </td>
                                                <td>{!! \App\Models\Config::showAuthor($slide->id_user) !!}</td>
                                                <td>{{ date_format($slide->created_at,'d/m/Y H:i:s') }}</td>
                                                <td>
                                                    <a href="{{ route('admin.slides.edit',$slide->id) }}"
                                                       class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i></a>
                                                    <a href="#" name="{{ $slide->title }}"
                                                       link="{{ route('admin.slides.destroy',$slide->id) }}"
                                    ...
                                    @if($slides->links())
                                        <div class="col-12 col-md-7">
                                            <div class="float-right">
                                                <div class="pagination">{!! $slides->links('pagination::bootstrap-4') !!}</div>
                                            </div>
                                        </div>
                                    @endif
                                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $(".deleteClick").click(function () {
                ...
                $("#deleteMes").html("Bạn muốn xoá slide <b>" + name + "</b> ?");
            });
            ...
            $('.btnDeleteAll').on('click', function () {
                ...
                if ($delete != "") {
                    $.ajax({
                        url: "{{ route('admin.slides.deleteAll') }}",
                        type: "post",
                        dataType: "text",
