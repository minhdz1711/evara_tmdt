                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.menus.store') }}" method="POST">
                                ...
                                <div class="form-group">
                                    @include('admins.layouts.button-add',['url'=>route('admin.menus.index'),'display'=>'d-none'])
                                </div>
                            </form>
                        </div>
                    </div>
                                ...
                                <table id="example1" class="table table-bordered table-striped dataTable">
                                    ...
                                    @if(count($menus)>0)
                                        @foreach($menus as $menu)
                                            <tr class="remove-{{ $menu->id }}">
                                                <td class="center">
                                                    <div class="custom-control custom-checkbox">
                                                        <input class="custom-control-input ace mycheckbox"
                                                               type="checkbox"
                                                               id="checkall-{{ $menu->id }}"
                                                               value="{{ $menu->id }}" name="check[]">
                                                        <label for="checkall-{{ $menu->id }}"
                                                               class="custom-control-label"></label>
                                                    </div>
                                                </td>
                                                <td>{{ $menu->title }}</td>
                                                <td>
                                                    {!! \App\Models\Config::getPositionMenu($menu->id_position) !!}
                                                </td>
                                                <td>
                                                    <div data-table="menus" data-id="{{ $menu->id }}"
                                                         data-col="is_active"
                                                         class="Switch Round  @if($menu->is_active == 1) On @else Off @endif"
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.menu-items.index',$menu->id) }}"
                                                       class="btn btn-sm btn-primary"><i class="fa fa-cog"></i> Menu Item</a>
                                                    <a href="{{ route('admin.menus.edit',$menu->id) }}"
                                                       class="btn btn-sm btn-warning"><i class="fa fa-pencil"></i></a>
                                                    <a href="#" name="{{ $menu->title }}"
                                                       link="{{ route('admin.menus.destroy',$menu->id) }}"
                                    ...
                                    @if($menus->links())
                                        <div class="col-12 col-md-6 col-lg-7">
                                            <div class="float-right">
                                                <div
                                                    class="pagination">{!! $menus->links('pagination::bootstrap-4') !!}</div>
                                            </div>
                                        </div>
                                    @endif
                                </div>
